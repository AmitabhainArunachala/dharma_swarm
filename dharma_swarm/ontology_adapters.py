...
def sync_all(
    ...
    for name, adapter_fn in _ADAPTERS:
        ...
        for obj, links in items:
            ...
            # Insert directly into the registry's object store
            # (We bypass create_object because we need deterministic IDs.)
            registry._objects[obj.id] = obj
            ingested += 1

            # Create links where both endpoints exist
            for link in links:
                src = registry.get_object(link.source_id)
                tgt = registry.get_object(link.target_id)
                if src is not None and tgt is not None:
                    # Check that the link def exists
                    link_def = registry.get_link_def(
                        link.source_type, link.link_name
                    )
