"""Governance filter for canonical provider events."""
...
from .events import CanonicalEvent, ThinkingComplete, ToolCallComplete
...
class GovernancePolicy:
    """Configurable governance rules for event stream processing."""
    ...
    gated_tools: set[str] = field(
        default_factory=lambda: {"Bash", "Write", "Edit", "NotebookEdit"}
    )
    auto_approved_tools: set[str] = field(
        default_factory=lambda: {"Read", "Glob", "Grep", "WebSearch", "WebFetch"}
    )
...
class GovernanceFilter:
    ...
    def process(self, event: CanonicalEvent) -> CanonicalEvent | None:
        """Return filtered event, or None to suppress event delivery."""
        ...
        # Generic content truncation/sanitization for events that carry text.
        if hasattr(event, "content"):
            ...
            if isinstance(content, str):
                ...
                if len(content) > self.policy.max_tool_output_chars:
                    ...
                    content = (
                        content[: self.policy.max_tool_output_chars]
    ...
    def _audit(self, action: str, event: CanonicalEvent) -> None:
        ...
        if self.policy.redact_thinking_in_audit and isinstance(event, ThinkingComplete):
            entry["thinking_hash"] = hashlib.sha256(
