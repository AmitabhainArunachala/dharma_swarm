...
export default function ObservatoryPage() {
  ...
  // Sort agents by composite_fitness for leaderboard
  const rankedAgents = useMemo(() => {
    if (!data?.agents) return [];
    return [...data.agents].sort((a, b) => b.composite_fitness - a.composite_fitness);
  }, [data?.agents]);
    ...
    >
      ...
      <div>
        <div className="flex items-center gap-3">
          <HeartPulse size={22} style={{ color: colors.botan }} />
          <h1
            className="font-heading text-2xl font-bold tracking-tight"
            style={{ color: colors.botan, textShadow: glowText(colors.botan, 0.5) }}
          >
            Agent Observatory
          </h1>
        </div>
        <p className="mt-1.5 text-sm" style={{ color: colors.sumi[600] }}>
          Fleet-wide health monitoring, fitness rankings, and activity stream
        </p>
      </div>
